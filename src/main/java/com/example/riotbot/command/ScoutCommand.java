...
=======
        ...
        public void init() {
            riotClient.getLatestDDragonVersion()
                ...
                .doOnError(e -> System.out.println("ScoutCommand: Failed to fetch DDragon version, using default " + latestVersion))
                .subscribe();
        }
...
=======
                                                                    ...
                                                                    return riotClient.getSummoner(account.puuid())
                                                                                                                                        .flatMap(summoner -> {
                                                                                                                                                // STEP 2: Get Match History IDs
                                                                                                                                                System.out.println("Step 2: Fetching recent match IDs for PUUID: " + account.puuid());
                                                                                                                                                return riotClient.getMatchIds(account.puuid(), 10)
                                                                                                                                                        .doOnNext(ids -> System.out.println("Step 2 Debug: Raw Match IDs found: " + ids))
                                                                                                                                                        .flatMapMany(Flux::fromIterable)
                                                                                                                                                        .flatMap(matchId -> riotClient.getMatchDetail(matchId)
                                                                                                                                                        ...
                                                                                                                                                        .flatMap(matches -> {
                                                                                                                                                                if (matches.isEmpty()) {
                                                                                                                                                                        return Mono.error(new RuntimeException("No recent matches found."));
                                                                                                                                                                }

                                                                                                                                                                // Smart Sort: Newest First
                                                                                                                                                                matches.sort((m1, m2) -> Long.compare(m2.info().gameEndTimestamp(), m1.info().gameEndTimestamp()));
                                                                                                                                                                MatchDto latestMatch = matches.get(0);
                                                                                                                                                                ...
                                                                                                                                                                System.out.println("Step 2 Success: Found " + matches.size() + " matches. Latest ID: " + latestMatch.metadata().matchId());

                                                                                                                                                                // Skip old Step 3 (Extraction) -> Go to Step 4
                                                                                                                                                                return Mono.just(new Step3Result(summoner, latestMatch, account));
                                                                                                                                                        });
                                                                                                                                        });
                                                                                                                        })
                                                                                                                        ...
                                                                                                                        // STEP 4: Verification - Get League Rank (NA1)
                                                                                                                        .flatMap(step3Result -> {
                                                                                                                                com.example.riotbot.dto.SummonerDto summoner = step3Result.summoner();
                                                                                                                                MatchDto latestMatch = step3Result.latestMatch();
                                                                                                                                AccountDto account = step3Result.account();

                                                                                                                                System.out.println("Step 4: Verifying ID via League-V4 for: " + summoner.id());
                                                                                                                                return riotClient.getLeagueEntries(summoner.id())
                                                                                                                                                .filter(l -> "RANKED_SOLO_5x5".equals(l.queueType()))
                                                                                                                                                .next()
                                                                                                                                                .map(l -> l.tier() + " " + l.rank())
                                                                                                                                                .defaultIfEmpty("Unranked")
                                                                                                                                                .onErrorResume(e -> {
                                                                                                                                                        System.out.println("Step 4 Failed (Rank Lookup): " + e.getMessage());
                                                                                                                                                        return Mono.just("Rank Unknown (API Error)");
                                                                                                                                                })
                                                                                                                                                .doOnNext(rank -> System.out.println("Step 4 Success: User is " + rank))
                                                                                                                                                .map(rank -> new Step4Result(summoner, latestMatch, rank, account));
                                                                                                                        })

                                                                                                                        // STEP 5: Get Active Game (Spectator V4 - NA1)
                                                                                                                        .flatMap(step4Result -> {
                                                                                                                                com.example.riotbot.dto.SummonerDto summoner = step4Result.summoner();
                                                                                                                                MatchDto latestMatch = step4Result.latestMatch();
                                                                                                                                String rank = step4Result.rank();
                                                                                                                                AccountDto account = step4Result.account();

                                                                                                                                System.out.println("Step 5: Calling Spectator V4...");
                                                                                                                                return riotClient.getCurrentMatch(summoner.id())
                                                                                                                                                .flatMap(activeGame -> {
                                                                                                                                                        System.out.println("Step 5 Success: Match Found!");
                                                                                                                                                        return processActiveGame(event, account, activeGame, rank);
                                                                                                                                                })
                                                                                                                                                // PLAN B: Use Latest Match Details
                                                                                                                                                .onErrorResume(e -> {
                                                                                                                                                        String errorMsg = e instanceof WebClientResponseException.NotFound
                                                                                                                                                                        ? "User is not currently in a game."
                                                                                                                                                                        : "Live Game not found (API Restricted).";
                                                                                                                                                        // Log only, don't show user
                                                                                                                                                        System.out.println("Step 5 Fallback: " + errorMsg + " Proceeding to Match History.");

                                                                                                                                                        // Directly use the latestMatch
                                                                                                                                                        return Mono.just(latestMatch)
                                                                                                                                                                        .flatMap(lastMatchDto -> {
                                                                                                                                                                                MatchDto.Participant foundParticipant = null;
                                                                                                                                                                                for (MatchDto.Participant p : lastMatchDto.info().participants()) {
                                                                                                                                                                                        if (p.puuid().equals(account.puuid())) {
                                                                                                                                                                                                foundParticipant = p;
                                                                                                                                                                                                break;
                                                                                                                                                                                        }
                                                                                                                                                                                }
                                                                                                                                                                                ...
                                                                                                                                                                                if (foundParticipant == null) {
                                                                                                                                                                                        return Mono.error(new RuntimeException("User not found in match participants"));
                                                                                                                                                                                }
                                                                                                                                                                                ...
                                                                                                                                                                                String thumbUrl = "https://ddragon.leagueoflegends.com/cdn/" + latestVersion + "/img/champion/" + championUrlName + ".png";
                                                                                                                                                                                
                                                                                                                                                                                // Identity (Author) - Using SummonerDTO Profile Icon
                                                                                                                                                                                String authorName = finalGameName + " #" + finalTagLine;
                                                                                                                                                                                int profileIconId = summoner.profileIconId();
                                                                                                                                                                                String profileIconUrl = "https://ddragon.leagueoflegends.com/cdn/" + latestVersion + "/img/profileicon/" + profileIconId + ".png";
                                                                                                                                                                                ...
                                                                                                                                                                                return event.editReply(discord4j.core.spec.InteractionReplyEditSpec.create()
                                                                                                                                                                                    .withContent("") // Silence "Not Found" message
                                                                                                                                                                                    .withEmbeds(embed))
                                                                                                                                                                                    .then();
                                                                                                                                                                        })
        ...
        private record Step3Result(com.example.riotbot.dto.SummonerDto summoner, MatchDto latestMatch, AccountDto account) {
        }

        private record Step4Result(com.example.riotbot.dto.SummonerDto summoner, MatchDto latestMatch, String rank, AccountDto account) {
>>>>>>> 21a077ed4ef3f73981e81d5138c2daeb961bae81
        }
